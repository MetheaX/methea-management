...
window.Vaadin.Flow['_vaadintheme_metheamanagement_globalCss'] = window.Vaadin.Flow['_vaadintheme_metheamanagement_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin.Flow['_vaadintheme_metheamanagement_globalCss'].length === 0) || (!window.Vaadin.Flow['_vaadintheme_metheamanagement_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin.Flow['_vaadintheme_metheamanagement_globalCss'].push(target);
  }
  if (!document['_vaadintheme_metheamanagement_componentCss']) {
    
    document['_vaadintheme_metheamanagement_componentCss'] = true;
  }
